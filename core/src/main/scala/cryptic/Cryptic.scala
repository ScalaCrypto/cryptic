...
object Cryptic {
  import Encrypted._
  sealed trait Operation[V] extends Cryptic[V] {
  ...
  final case class Mapped[V, W: Serializer](src: Cryptic[V], f: V => W) extends Operation[W] {
    ...
    def bytes(implicit encrypt: Encrypt, decrypt: Decrypt): Either[String, Array[Byte]] =
      run(encrypt, decrypt).map(_.bytes)
  }
  final case class FlatMapped[V, W: Serializer](src: Cryptic[V], f: V => Cryptic[W])
      extends Operation[W] {
    override def run(implicit encrypt: Encrypt, decrypt: Decrypt): Either[String, Encrypted[W]] =
      decrypted.map(w => Encrypted(encrypt(implicitly[Serializer[W]].serialize(w))))
