...
case class PlainText(bytes: IArray[Byte], aad: AAD):
  ...
  override def equals(obj: Any): Boolean = obj match
    case other: PlainText =>
      aad.sameElements(other.aad) && bytes.sameElements(other.bytes)
    case _ => false
...
object PlainText:
  ...
  def hash(plainText: PlainText): Hash =
    ...
    if plainText.aad.nonEmpty then digest.update(plainText.aad.mutable)
    digest.digest(plainText.bytes.mutable).immutable
...
extension [V: Codec](value: V)
  def encrypted(using encrypt: Encrypt): Encrypted[V] =
    Encrypted(value, AAD.empty)
  def encrypted(aad: AAD)(using encrypt: Encrypt): Encrypted[V] =
    Encrypted(value, aad)
...
object Encrypted:
  ...
  /** Encrypts a given value of type `V` using the provided additional
    * authentication data (AAD) and returns an `Encrypted[V]` instance.
    *
    * @param value
    *   The value to be encrypted. If the value is null, an empty `Encrypted[V]`
    *   instance is returned.
    * @param aad
    *   The additional authentication data (AAD) to be used during encryption.
    * @param encrypt
    *   The given encryption provider used to encrypt the value.
    * @return
    *   An `Encrypted[V]` instance containing the encrypted representation of
    *   the value.
    */
  def apply[V: Codec](
      value: V,
    ...
    else Encrypted[V](encrypt(summon[Codec[V]].encode(value, aad)))
