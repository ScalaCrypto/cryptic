...
/** Command-line interface for the Enigma encryption/decryption tool.
  *
  * Provides functionality to encrypt or decrypt messages using specific
  * settings. The input can be provided directly via text or read from a file.
  *
  * Arguments:
  *
  *   - `-d, --decrypt`: Specifies decryption mode. Encrypt mode is the default.
  *   - `-s, --settings SETTINGS`: Specifies the Enigma settings as a single
  *     string, e.g., "III-II-I AAA AAZ B ABCD". This is optional if the
  *     environment variable `ENIGMA_SETTINGS` is set. This argument overrides
  *     the environment variable if provided.
  *   - `-f, --file FILE`: Reads the input message from a specified file.
  *   - `TEXT`: The input message text (optional if the input is provided via a
  *     file or stdin).
  *
  * Notes:
  *
  *   - Input text is normalized to uppercase letters A–Z, ignoring
  ...
  * Methods:
  *
  *   - `run`: Parses and processes command-line arguments for encryption or
  *     decryption.
  *   - `main`: Entry point for the application. Accepts and processes
  ...
  *   - `process`: Determines encryption or decryption logic based on the
  *     settings.
  ...
  * Extensions:
  *
  *   - `group5`: String extension method to format output into groups of five
  *     letters.
  */
object cli:
