...
lazy val core = (project in file("core")).settings(
  ...
  artifactName := { (sv: ScalaVersion, module: ModuleID, artifact: Artifact) =>
    s"cryptic-${artifact.name}-${module.revision}-${artifact.`type`}.${artifact.extension}"
      .replace("-jar", "")
  }
)
...
lazy val `serialization-fst` = (project in file("serialization-fst"))
  .settings(
    ...
    artifactName := { (sv: ScalaVersion, module: ModuleID, artifact: Artifact) =>
      s"cryptic-${artifact.name
        .replace("serialization-", "")}-${module.revision}-${artifact.`type`}.${artifact.extension}"
        .replace("-jar", "")
    }
  )
  .dependsOn(core)
  ...
  .settings(
    ...
    artifactName := {
      (sv: ScalaVersion, module: ModuleID, artifact: Artifact) =>
        s"cryptic-${artifact.name.replace("crypto-", "")}-${module.revision}-${artifact.`type`}.${artifact.extension}"
          .replace("-jar", "")
    }
  )
  .dependsOn(core)
