...
/** Provides default cryptographic utilities and codec implementations.
  ...
  *   - Encryption and decryption operations specific to the `RsaAes` cipher
  *     from `cryptic.cipher.RsaAes`.
  *   - Key pair generation utilities for RSA encryption from
  *     `cryptic.cipher.Rsa`.
  *
  * The `encrypt` and `decrypt` operations exposed in this object are limited to
  * the `RsaAes` cipher.
  */
