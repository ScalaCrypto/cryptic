...
class EncryptedSpec extends AnyFlatSpec with Matchers with TryValues {
  ...
  "Encrypted" should "be collectable" in {
    enc1.collect { case `clear` => "nice!" }.decrypted shouldBe Success("nice!")
    enc1.collect { case "kalle" => "nice!" }.run shouldBe Success(emptyString)
    enc1
      .collect { case "kalle" => "nice!" }
      .decrypted
      .failure
      .exception
  ...
  "Encrypted" should "be foldable" in {
    ...
    enc1.filter(_.length > 10).run.flatMap { _.fold("kalle") { _.toUpperCase } } shouldBe Success("kalle")
    enc1.filter(_.length < 10).run.flatMap { _.fold("kalle") { _.toUpperCase } } shouldBe Success("NISSE")
  }
  ...
  "Encrypted" should "be or-elsable" in {
    enc1.filter(_.length > 2).orElse("kalle".encrypted).decrypted shouldBe Success(clear)
    enc1.filter(_.length < 2).orElse("kalle".encrypted).decrypted shouldBe Success("kalle")
  }
}
