...
/** FST (Fast Serialization) based Codec for arbitrary values.
  *
  * Uses a shared `FSTConfiguration` to encode values to bytes and decode them
  * back.
  *
  * Notes:
  *   - Encoding stores FST-produced bytes into `PlainText.bytes` and passes the
  *     given `AAD` through without interpretation.
  *   - Decoding reads from `PlainText.bytes`; the AAD is ignored by this codec.
  *   - Binary wire format; not human-readable. Ensure compatible classpaths and
  *     FST configuration on both ends.
  */
