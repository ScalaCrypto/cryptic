...
sealed abstract class Encrypted[V: Serializer] {
  ...
  def filter(pred: V => Boolean)(implicit optionSerializer: Serializer[Option[V]]): Encrypted[Option[V]] = Filtered(this, pred)
...
object Encrypted {
  ...
  case object Empty extends Encrypted[Nothing] {
    ...
    override def filter(pred: Nothing => Boolean)(implicit optionSerializer: Serializer[Option[Nothing]]): Encrypted[Option[Nothing]] = empty
  ...
  final case class Filtered[V: Serializer](e: Encrypted[V], pred: V => Boolean)(implicit optionSerializer: Serializer[Option[V]]) extends Encrypted[Option[V]] {
    override def run(implicit encrypt: Encrypt, decrypt: Decrypt): Either[String, Encrypted[Option[V]]] =
      decrypted.map(_.map(implicitly[Serializer[V]].serialize) match {
        case Some(pt) => Value[Option[V]](encrypt(pt)) // TODO: This looks quite suspicious, will it fail at decryption?
        case None => empty
      })
  ...
  final case class FlatMapped[V, W: Serializer](e: Encrypted[V], f: V => Encrypted[W]) extends Encrypted[W] {
    override def run(implicit encrypt: Encrypt, decrypt: Decrypt): Either[String, Encrypted[W]] =
      decrypted.map(w => Value(encrypt(implicitly[Serializer[W]].serialize(w))))
